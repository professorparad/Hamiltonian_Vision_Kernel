...
def run(args: argparse.Namespace) -> None:
    ...
    for image_path in image_paths(args.dataset_dir, args.count):
        ...
        rows.append(
            read_gan_metrics(
                image_output / "gan_summary.json",
                image_output / "gan_training_history.csv",
                image_path.name,
            )
        )
    summary = write_metric_outputs(args.output_dir, rows)
    print(f"GAN CIFAR-10 summary: {summary}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run GAN on CIFAR-10 subset.")

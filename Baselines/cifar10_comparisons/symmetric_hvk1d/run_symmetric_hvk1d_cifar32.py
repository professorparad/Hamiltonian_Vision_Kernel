"""
U(1)-symmetric HVK1D on CIFAR-10 at native 32×32.
Energy = J*ZZ + K*(XX + YY) with learnable J, K per bond.
"""
...
def train_symmetric_hvk1d(
    ...
    for step in range(epochs):
        ...
        if step % 50 == 0 or step == epochs - 1:
            print(f"  Symm-HVK1D Step {step:>4d}: loss={loss.item():.6f} recon={recon_loss.item():.6f}")
...
def run(args: argparse.Namespace) -> list[dict]:
    ...
    for img_path in paths:
        print(f"\nSymm-HVK1D processing: {img_path.name}")
        image = load_grayscale_image(img_path)
        reconstruction, order_rows = train_symmetric_hvk1d(image, device, args.epochs)
        metrics = compute_metrics(reconstruction, image)
        rows.append({
            "model": "SymmetricHVK1D",
            "image": img_path.name,
        ...
        stem = img_path.stem
        write_csv(output_dir / f"{stem}_symmetric_hvk1d_order_parameters.csv", order_rows)
        save_order_parameter_plot(
            order_rows,
            output_dir / f"{stem}_symmetric_hvk1d_order_parameters.png",
            f"Symmetric HVK1D CIFAR order parameters: {img_path.name}",
        )
        save_reconstruction_comparison(
            image,
            reconstruction,
            output_dir / f"{stem}_symmetric_hvk1d_reconstruction.png",
            f"Symmetric HVK1D CIFAR reconstruction: {img_path.name}",
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Symmetric HVK1D on CIFAR-10 at 32×32.")
...
if __name__ == "__main__":
    ...
    rows = run(args)
    output_dir = Path(__file__).resolve().parent / "outputs"
    write_csv(output_dir / "symmetric_hvk1d_cifar32_metrics.csv", rows)
    print(f"\nResults saved to {output_dir / 'symmetric_hvk1d_cifar32_metrics.csv'}")

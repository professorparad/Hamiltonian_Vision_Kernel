...
RESULTS_PATH = Path(__file__).resolve().parent / "outputs" / "checkpoint_bond_dimension_temperature_hardware_sweep" / "ibm_ibm_marrakesh_shots1024.json"
OUTPUT_PATHS = [
    Path(__file__).resolve().parents[1] / "latex_outputs" / "paper_latex" / "figures" / "checkpoint_hardware_bond_dimension_r_es.pdf",
    Path(__file__).resolve().parent / "outputs" / "checkpoint_bond_dimension_temperature_hardware_sweep" / "checkpoint_hardware_bond_dimension_r_es.png",
]
...
def main() -> None:
    ...
    fig.suptitle("R_ES vs epoch across MPS bond dimension, real trained checkpoints on ibm_marrakesh (1024 shots)")
    fig.tight_layout()

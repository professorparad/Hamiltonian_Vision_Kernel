...
def save_plot(rows: list[dict], dataset_name: str, shots: int, backend: str, output_path: Path) -> None:
    ...
    ax.set_title(f"Order parameter vs epoch ({dataset_name}, real IBM hardware, shots={shots}, {backend})")
    ax.set_xlabel("Epoch")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Submit real-hardware epoch-labeled HVK circuits across n_qubits=4/6/8 for each shots budget."
    )
...
def main() -> None:
    ...
    if args.dry_run:
        for shots in shots_list:
            print(f"[dry-run] would submit {len(circuits)} circuits at shots={shots}")
        return

    for shots in shots_list:
        backend, job_id, result = run_on_ibm(circuits, args.backend, shots, args.token, max_n_qubits)
        rows = []
        ...
        json_path = args.output_dir / f"real_hardware_shots{shots}.json"
        json_path.write_text(json.dumps(rows, indent=2), encoding="utf-8")
        print(f"shots={shots}: backend={backend} job_id={job_id} results={json_path}")

        for dataset_name in DATASETS:
            plot_path = args.output_dir / f"{dataset_name}_real_hardware_shots{shots}_order_parameter_vs_epoch.png"

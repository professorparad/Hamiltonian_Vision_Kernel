import numpy as np 
import torch 
def sinusodial_encoding(positions:np.ndarray , D_encoded_vector_space : int = 8 , base : float = 10000.0 , device = None ) -> torch.tensor:
    if D_encoded_vector_space % 4 != 0:
        raise ValueError("D_encoded_vector_space must be divisible by 4 .") 
    positions = np.asarray(positions , dtype=np.float32)
    if positions.ndim != 2 or positions.shape[1] != 2:
        raise ValueError("Positoions must have shape( N , 2).")
    x = positions[: ,0]
    y = positions[: , 1]
    n_positions = positions.shape[0]
    n_bands = D_encoded_vector_space // 4 
    encoding = np.zero((n_positions , D_encoded_vector_space) , dtype = np.float32)

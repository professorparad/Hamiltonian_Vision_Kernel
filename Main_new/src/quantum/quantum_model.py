...
class QuantumModel(nn.Module):
    def __init__(
        ...
        if use_classical_replacement:
            ...
            # Raw, unconstrained coupling parameters. Bounded via tanh in `couplings`
            # below: unlike the legacy model, these can no longer grow without limit
            # to trivially minimize a linear energy loss independent of reconstruction
            # quality (see Main_new/README.md for the diagnosis this fixes).
            self.Jx_raw = nn.Parameter(0.1 * torch.randn(self.n_bonds))
            self.Jy_raw = nn.Parameter(0.1 * torch.randn(self.n_bonds))
            self.Jz_raw = nn.Parameter(0.1 * torch.randn(self.n_bonds))
    ...
    @property
    def Jx(self) -> torch.Tensor:
        return torch.tanh(self.Jx_raw)

    @property
    def Jy(self) -> torch.Tensor:
        return torch.tanh(self.Jy_raw)

    @property
    def Jz(self) -> torch.Tensor:
        return torch.tanh(self.Jz_raw)

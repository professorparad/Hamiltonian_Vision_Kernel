...
class Quantum2DGridModel(nn.Module):
    def __init__(self, feature_dim: int, positional_dim: int):
        ...
        # Raw, unconstrained coupling. Bounded via tanh in the `j_2d` property below:
        # the original j_2d was a free parameter feeding only the energy loss (never
        # the decoder's observables), so it could grow without limit to trivially
        # minimize a linear energy loss independent of reconstruction quality. See
        # Main_new2/README.md.
        self.j_2d_raw = nn.Parameter(0.1 * torch.randn(len(ALL_EDGES)))

    @property
    def j_2d(self) -> torch.Tensor:
        return torch.tanh(self.j_2d_raw)

...
from Main_new2.src.analysis import (
    detect_phase_transition,
    main2_correlation_rows,
    main2_order_summary,
)
from Main_new2.src.config import Main2Config
from Main_new2.src.dataset import build_main2_dataset
from Main_new2.src.model import PatchDecoder, Quantum2DGridModel
from Main_new2.src.outputs import (
    save_gif,
    save_order_curve,
    save_reconstruction_frame,
    write_csv,
)
from Main_new2.src.pathing import add_main_package_to_path
...
def run_main2(config: Main2Config):
    if config.energy_loss_mode not in VALID_ENERGY_LOSS_MODES:
        raise ValueError(
            f"Unknown energy_loss_mode '{config.energy_loss_mode}'. "
    ...
    for step in range(config.steps + 1):
        ...
        elif config.energy_loss_mode == "positive":
            energy_loss = torch.mean(energies.square())
        else:
            permutation = torch.randperm(data["positions"].shape[0], device=device)
            _, negative_energies = model(data["features"], data["positions"][permutation])
            energy_loss = F.softplus(energies - negative_energies + config.energy_margin).mean()

        loss = reconstruction_loss + config.energy_weight * energy_loss
        loss.backward()
        optimizer.step()
        ...
        if step % 20 == 0:
            print(
                ...
                f"2D Energy ({config.energy_loss_mode}): {energy_loss.item():.6f}"
            )
    ...
    return {
        ...
        "final_reconstruction_mse": epoch_rows[-1]["reconstruction_mse"] if epoch_rows else None,
    }

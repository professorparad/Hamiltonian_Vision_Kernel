...
During training, each image patch is represented by a variational quantum state whose measured observables are recorded at selected epochs. Let $n\in\{1,\ldots,N\}$ index image patches, $q\in\{1,\ldots,Q\}$ index qubits, and $t$ index the training epoch. In the Monalisa experiment $N=16$ and $Q=6$; in the CIFAR-10 benchmark the images are resized to the same $4\times4$ patch grid, so the same normalization applies. The local measured observables are
\begin{equation}
Z_{nq}(t)=\langle Z_q\rangle_{n,t},
\qquad
X_{nq}(t)=\langle X_q\rangle_{n,t}.
\end{equation}
The patch-level longitudinal magnetization is the qubit average
\begin{equation}
m_n^{z}(t)
=
\frac{1}{Q}
\sum_{q=1}^{Q} Z_{nq}(t),
\end{equation}
and the global longitudinal order parameter is the patch average
\begin{equation}
M_z(t)
=
\frac{1}{N}
...
\sum_{n=1}^{N}
\sum_{q=1}^{Q}Z_{nq}(t).
\label{eq:global_order}
\end{equation}
The factor $1/(NQ)$ is important: it makes $M_z(t)$ an intensive quantity, so its magnitude does not grow simply because the number of patches or qubits is increased. Since each Pauli expectation lies in $[-1,1]$, the normalized order parameter also satisfies $M_z(t)\in[-1,1]$.

Two auxiliary order statistics are recorded. The absolute order parameter
\begin{equation}
|M_z|(t)
=
\frac{1}{N}
\sum_{n=1}^{N}\left|m_n^{z}(t)\right|
\end{equation}
measures the strength of longitudinal ordering independently of sign, while the transverse order parameter
\begin{equation}
M_x(t)
=
\frac{1}{NQ}
...
\sum_{q=1}^{Q}X_{nq}(t)
\end{equation}
tracks alignment along the $X$ sector. Together, $M_z(t)$, $|M_z|(t)$, and $M_x(t)$ distinguish sign-coherent longitudinal ordering, unsigned ordering strength, and transverse spin alignment.

The lattice-correlation statistic is normalized by the number of patches and by the number of Hamiltonian interaction channels, so it is also intensive. For the 1D HVK, the chain graph has $B=Q-1=5$ nearest-neighbor bonds and three anisotropic Heisenberg sectors. With
\begin{equation}
ZZ_{nb}(t)=\langle Z_bZ_{b+1}\rangle_{n,t},
\quad
XX_{nb}(t)=\langle X_bX_{b+1}\rangle_{n,t},
\quad
YY_{nb}(t)=\langle Y_bY_{b+1}\rangle_{n,t},
\end{equation}
the recorded 1D lattice-correlation order statistic is
\begin{equation}
C_{1D}(t)
=
\frac{1}{3NB}
...
\sum_{b=1}^{B}
\left[
ZZ_{nb}(t)+XX_{nb}(t)+YY_{nb}(t)
\right].
\label{eq:1d_correlation_order}
\end{equation}
For the 2D HVK, the qubits are arranged on a $2\times3$ grid with edge set
\begin{equation}
\mathcal{E}
=
\mathcal{E}_h\cup\mathcal{E}_v,
\qquad
|\mathcal{E}|=7.
\end{equation}
The 2D correlation order statistic is therefore
\begin{equation}
C_{2D}(t)
=
\frac{1}{N|\mathcal{E}|}
...
\sum_{(u,v)\in\mathcal{E}}
\langle Z_uZ_v\rangle_{n,t}.
\label{eq:2d_correlation_order}
\end{equation}
Equations~\eqref{eq:1d_correlation_order} and~\eqref{eq:2d_correlation_order} explain the scaling behavior: adding more patches, qubits, or bonds changes the number of measured terms, but the normalization converts the raw sum into a mean correlation per patch per interaction channel. This allows the 1D and 2D trajectories to be compared qualitatively even though their Hamiltonian graphs are different.

Mean Hamiltonian energy is recorded separately as
\begin{equation}
\overline{E}(t)
=
\frac{1}{N}
\sum_{n=1}^{N}E_n(t).
\end{equation}
The energy is not used directly as the order parameter because the 1D and 2D Hamiltonians have different coupling graphs and different learned coupling tensors. Instead, the order parameters are computed from normalized observable averages, while $\overline{E}(t)$ is treated as a model-specific descent diagnostic.

To detect a phase-transition-like event during optimization, the code uses a discrete susceptibility signal based on the epoch-to-epoch change of the global order parameter. To avoid confusion with the MPS bond dimension $\chi$, denote this susceptibility by
\begin{equation}
\mathcal{X}(t)
=
\left|M_z(t)-M_z(t-\Delta t)\right|,
\end{equation}
where $\Delta t$ is the spacing between recorded epochs. In the runs reported here, order parameters are recorded at each saved epoch, so $\Delta t=1$ for the primary Monalisa runs. The candidate critical epoch is the peak-susceptibility epoch
\begin{equation}
t_c
=
\arg\max_t \mathcal{X}(t),
\qquad
\mathcal{X}_{\max}
=
\max_t \mathcal{X}(t).
\end{equation}
A transition is declared only if this peak exceeds a robust baseline threshold:
\begin{equation}
\tau
=
\operatorname{median}\{\mathcal{X}(t)\}_t
+2\,\operatorname{std}\{\mathcal{X}(t)\}_t,
\qquad
\mathcal{X}_{\max}>\tau.
\label{eq:transition_threshold}
\end{equation}
The reported order-parameter jump is
\begin{equation}
\Delta M_z(t_c)
=
\left|M_z(t_c)-M_z(t_c-\Delta t)\right|
=
\mathcal{X}(t_c).
\end{equation}

This is a finite-size phase-transition diagnostic, not a claim of a thermodynamic singularity. A true equilibrium phase transition would require a control parameter, an infinite-size limit, and non-analytic behavior of a free-energy density. HVK instead observes a finite learned system during gradient optimization. Therefore, the phrase ``phase transition'' means a statistically sharp reorganization of the learned observable manifold: a sudden jump in normalized magnetization, accompanied by a susceptibility peak above the median-plus-two-standard-deviation threshold in Equation~\eqref{eq:transition_threshold}. Under this definition, the 1D HVK detects a later and smaller reorganization, while the 2D HVK detects an earlier and larger one, consistent with the stronger spatial coupling bias of the 2D interaction graph.
...
The experiments lead to several methodological observations about the role of the Hamiltonian Vision Kernel as a reconstruction architecture. These observations are stated cautiously because the reported experiments are finite-size simulations on a limited image set, rather than asymptotic claims about a universal quantum advantage.

\subsection{Tensor-Network Features Provide A Structured Input Basis}

The MPS feature stage contributes more than dimensionality reduction. By extracting local Pauli expectations, nearest-neighbor correlations, and entanglement entropies, the encoder converts each image patch into a feature vector whose coordinates already carry quantum-statistical meaning. This is important because the variational circuit is not asked to discover all local structure from raw pixels. Instead, it receives a compact representation in which patch-local amplitudes, correlations, and bipartite structure have already been separated into interpretable channels.

The bond-dimension sweep supports this interpretation. Increasing the MPS bond dimension improves direct patch reconstruction before the Hamiltonian decoder is applied, showing that the tensor-network representation controls the amount of local visual structure available to the downstream quantum circuit. The HVK decoder then operates on a latent space that is not arbitrary, but organized by the tensor-network preprocessing step.

\subsection{Spatial Positional Encoding Breaks Patch Degeneracy}

The positional encoding is necessary because MPS features alone are content-based. Two visually similar patches from different regions of the image may have similar tensor-network descriptors, even though their correct role in the global image is different. The sinusoidal coordinate map and learned projection into qubit rotations break this degeneracy by conditioning the quantum state on patch location.

This mechanism should be understood as a spatial inductive bias rather than as a post-hoc coordinate tag. Since the positional angles enter the circuit before measurement, the resulting observables are position-conditioned quantum features. Consequently, the decoder receives both patch-content structure and a learned representation of where that patch belongs in the image grid.

\subsection{Hamiltonian Energy Acts As A Latent Regularizer}

The energy term does not replace reconstruction loss. Instead, it regularizes the latent observable manifold by encouraging the measured correlators to move toward low-energy configurations of the learned Hamiltonian. The negative energy descent observed in both the 1D and 2D variants indicates that the model is not only reducing pixel-level error, but also organizing its internal observables according to the learned spin-correlation structure.

This distinction is methodologically important. A classical autoencoder can reconstruct an image but offers limited physical interpretation of its latent coordinates. HVK produces reconstruction outputs while also exposing energy, magnetization, correlation, and susceptibility signals. These quantities provide a second diagnostic layer for understanding how the model reorganizes during training.

\subsection{Topology Alignment Improves The Spatial Bias}

The comparison between the 1D and 2D HVK variants suggests that the circuit interaction graph matters. The 1D chain is a useful control because it measures the effect of a nearest-neighbor Heisenberg structure without matching the two-dimensional geometry of the image grid. The 2D HVK introduces horizontal and vertical interactions, making the Hamiltonian graph more consistent with image-patch adjacency.

The observed reduction in final MSE and the lower final Hamiltonian energy of the 2D variant are consistent with this topology-alignment principle. The result does not imply that the 2D graph is universally superior for every dataset or patch scale. It does indicate that, for the present benchmark, aligning the latent interaction graph with the data geometry improves reconstruction and produces stronger low-energy organization.

\subsection{Finite-Size Phase Diagnostics Should Be Interpreted Conservatively}

The detected critical epochs should be interpreted as optimization diagnostics rather than equilibrium phase transitions. The HVK system has a finite number of patches, a finite number of qubits, learned couplings, and a gradient-descent trajectory rather than a thermodynamic control parameter. Therefore, the susceptibility peak identifies a sharp reorganization in the trained observable field, not a singularity in a free-energy density.

This conservative interpretation is still useful. The susceptibility signal marks epochs where the magnetization-like order parameter changes abruptly, and these epochs coincide with rapid qualitative improvement in the reconstruction frames. Thus, the phase-transition language is best understood as a finite-size diagnostic vocabulary for latent-state reorganization.
...
2D HVK & $8.6420\times10^{-5}$ & $3.7584\times10^{-5}$ & $42.92$ & -- & $-6.9125$ & $60.3$ & -- & -- \\
...
\texttt{Baselines/cifar10\_benchmark/hvk2d/outputs/aggregate\_metrics.csv},
and
...
\caption{Reconstruction metric comparison for 1D HVK, 2D HVK, and the GAN baseline on the Monalisa image reconstruction task. The left panel shows MSE descent over training on a logarithmic scale. The right panel compares final MSE and the corresponding PSNR. Lower MSE and higher PSNR indicate better reconstruction quality.}
\label{fig:hvk_gan_metric_comparison}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{Baselines/outputs/cifar10_metric_comparison.png}
\caption{CIFAR-10 test subset reconstruction metric comparison for 1D HVK, 2D HVK, and the GAN baseline. The left panel shows average MSE descent over 200 training epochs. The right panel compares the average final MSE and the corresponding PSNR across the 10-image test subset.}
\label{fig:cifar10_metric_comparison}
\end{figure}

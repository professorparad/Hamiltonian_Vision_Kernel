...
\subsection{Training-Dynamics Reorganization: A Corrected, Partial Multi-Seed Result}
...
Table~\ref{tab:phase_transition_corrected} reports the result for three of six datasets (CIFAR-10, MNIST, Fashion-MNIST; two images $\times$ two seeds each, twelve runs total), with PathMNIST, BloodMNIST, and PneumoniaMNIST rerunning under the same corrected protocol at the time of writing. Unlike the withdrawn claim, detection is not universal: MNIST shows a detected reorganization in all four runs, while CIFAR-10 and Fashion-MNIST each detect one in only two of four. We report this partial, mixed result deliberately --- a training-dynamics diagnostic that only sometimes triggers is a more credible finding than one that always does, and we would rather publish four honestly-measured datasets than six of mixed provenance.

\begin{table}[t]
\centering
\caption{Corrected, noise-free training-dynamics diagnostic (2 images $\times$ 2 seeds/dataset). Three of six datasets completed at time of writing; PathMNIST, BloodMNIST, PneumoniaMNIST are in progress under the identical protocol.}
...
\begin{itemize}
    ...
    \item The original exploratory training-dynamics traces were withdrawn after a provenance audit (Section~\ref{sec:phase_transition}); the corrected, noise-free multi-seed replacement is a small-sample (2 images $\times$ 2 seeds), partial result (three of six datasets at time of writing) with mixed, not universal, detection, and should be read at that scope.

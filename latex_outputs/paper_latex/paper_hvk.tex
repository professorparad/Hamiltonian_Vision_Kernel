...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/phase_transition_single_display.pdf}
\caption{A single, unified view of the phase transition: order parameter $M_z(t)$ (left axis) and susceptibility $\mathcal{X}(t)$ (right axis) for the HVK2D entangling baseline on a shared training-epoch axis, with the detected transition point ($t_c=10$, $\mathcal{X}_{\max}=0.204$) marked directly on the susceptibility peak.}

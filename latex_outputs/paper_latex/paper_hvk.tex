...
\noindent\textbf{6) Training-dynamics diagnostics, in simulation and on real hardware:} A magnetization-style change-point statistic and an energy-to-entanglement ratio are tracked across training, varied along qubit-count ($N=4,6,8$) and MPS-bond-dimension ($\chi=1,2,4,8$) axes, and replayed via gate-for-gate checkpoint reconstruction on real IBM Quantum hardware and the IonQ simulator across three shot budgets (Sections~\ref{sec:phase_transition}--\ref{sec:checkpoint_hardware_bond_dimension_energy_entanglement}). These are descriptive, small-sample diagnostics --- not inferential evidence of a thermodynamic transition --- reported with the same detection formalism throughout (Eq.~\ref{eq:critical_epoch}).

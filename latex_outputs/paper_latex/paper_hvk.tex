...
\subsection{Training-Dynamics Provenance Audit}
\label{sec:phase_transition}
We audited the provenance of the exploratory order-parameter plots before treating them as evidence. The restricted-diagnostic trajectories were analytic interpolations produced for visualization rather than measurements from saved epoch checkpoints. The 13 multi-dataset traces were measured during training, but HVK2D's training-mode forward pass deliberately adds Gaussian observable noise, so their largest one-step changes confound parameter evolution with injected jitter. We therefore exclude both sets of plots and all associated change-point counts from the paper's results and conclusions. The corrected runner now evaluates a noise-free, evaluation-mode forward pass after each optimizer update; a fresh multi-seed rerun is required before any training-reorganization claim is reconsidered.
...
\begin{itemize}
    ...
    \item The exploratory training-dynamics traces are excluded from evidentiary claims after the provenance audit of Section~\ref{sec:phase_transition}; deterministic multi-seed traces are required before this question can be revisited.

...
\begin{table}[H]
\centering
\caption{Matched core component ablations on the \textit{Monalisa} benchmark (five seeds, 240 training steps for every variant). The final column is a descriptive comparison with observed run-to-run variation, not a formal equivalence test.}
...
No MPS & $(4.9636\pm0.1727)\times10^{-4}$ & $33.04\pm0.15$ & $0.9933\pm0.0002$ & $+0.08$ & Within descriptive variability band \\
No entanglement & $(5.0345\pm0.1797)\times10^{-4}$ & $32.98\pm0.15$ & $0.9932\pm0.0002$ & $+0.02$ & Within descriptive variability band \\
...
\caption{Order parameter and mean Hamiltonian energy (left) and order-parameter susceptibility (right) for the same contrastive Hamiltonian-core run. The energy diagnostic drifts steadily negative while the order parameter stays comparatively flat after an early reorganization near epoch 10. This is consistent with the energy term acting as a diagnostic signal without being the quantity that most improves reconstruction, matching the conditional-value reading of Table~\ref{tab:hamiltonian_controls}.}
...
\subsection{Full Training-Reorganization Ablation}
\label{sec:restricted_ablation}
The main paper's restricted pair-correlation diagnostic (entangling observables representationally necessary for a nonlocal-structure target, mean $R^2=0.9735$ vs.\ $R^2\leq0.02$ for non-entangling controls) also has a training-dynamics signature. Table~\ref{tab:phase_transition_ablation} gives the per-variant candidate reorganization epoch $t_c$, peak susceptibility $\mathcal{X}_{\max}$, and final order parameter $M_z(200)$. Every variant peaks at the same early epoch, but the entangling channel has a $2.1$--$6.7\times$ larger susceptibility peak than any resource-matched classical control and a final order parameter roughly double the strongest control's. These are finite-size optimization diagnostics, not thermodynamic phase-transition evidence.
...
\begin{table}[H]
\centering
\caption{Training-reorganization ablation on the restricted pair-correlation diagnostic.}
...
We extend the same order-parameter/critical-epoch diagnostic to a small representative sample of images from CIFAR-10 and five further real datasets, training a fresh HVK2D model per image (non-overlapping $8\times8$ patches, 16 patches/image) and tracking $M_z(t)$ and $\mathcal{X}(t)=|M_z(t)-M_z(t-\Delta t)|$ at every step, applying the same median-plus-two-standard-deviation detection threshold used throughout this project. Table~\ref{tab:multi_dataset_phase_transition} reports the result: all 13 sampled images across the six datasets show a detected reorganization. This is a small-sample (2--3 images/dataset), single-seed diagnostic, reported at that scope; it shows that the signature recurs in the sampled real-image runs rather than only in the synthetic restricted diagnostic.

Rather than a single combined trajectory, Figure~\ref{fig:multi_dataset_gallery} shows the order parameter and susceptibility in the same dual-axis style as the main paper's examples, individually for \emph{every one of the 13 sampled images}. Each image is identified by its real class or content label rather than a bare index. Each panel marks its candidate reorganization epoch $t_c$ and peak susceptibility $\mathcal{X}_{\max}$.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/multi_dataset_phase_transition_gallery.pdf}
\caption{Order-parameter reorganization display for the 13 sampled images underlying Table~\ref{tab:multi_dataset_phase_transition}. Each panel plots $M_z(t)$ (solid) and $\mathcal{X}(t)$ (dashed), with the candidate reorganization epoch $t_c$ and peak susceptibility annotated. All 13 panels meet the stated operational criterion; this is a descriptive sampled result.}
\label{fig:multi_dataset_gallery}
\end{figure}

\begin{table}[H]
\centering
\caption{Order-parameter reorganization diagnostic across CIFAR-10 and five further real datasets (2--3 images/dataset, 100--200 training epochs, HVK2D). Every sampled image meets the stated susceptibility-peak criterion.}
...
The two topologies give close held-out reconstruction means at this scale. Across the 9 image--seed pairs, the paired HVK1D-minus-HVK2D difference is $0.16$~dB. Its bootstrap 95\% CI is $[-0.32,0.64]$~dB. After aggregating the three images within each seed, the two-sided Wilcoxon test gives $p=0.50$. The 3-seed, 3-image subset is too small to establish formal equivalence. HVK2D nevertheless completes the same 90-step budget in roughly half the wall-clock time of HVK1D at a comparable parameter count, a measured resource-cost difference. This is consistent with HVK2D's smaller measured observable set (19 vs.\ 27); the corresponding transpiled depth and two-qubit-gate counts are shown in Figure~\ref{fig:ibm_circuit_summary}. At the tested scale, the evidence supports treating topology primarily as a resource-cost choice rather than claiming an accuracy difference.
...
\section{Held-Out Output-Level $D_4$ Symmetry Evaluation}
\label{sec:d4_symmetry_experiment}
The main paper's $D_4$-equivariance claim (pooled HVK2D observable map, equivariance error $9.57\times10^{-17}$) was previously demonstrated on a single pass over five images, with no held-out protocol, no classical baseline sharing the same pooling construction, and no measurement of what equivariance costs or buys at the output level. This section closes that gap with a held-out, multi-seed (5 seeds, 20 train / 10 held-out images per seed, disjoint CIFAR-10 splits) experiment across five modes: the provably-equivariant \emph{D4-pooled-HVK2D} map; a new \emph{D4-pooled-classical} baseline applying the identical Reynolds-averaging construction (group-averaging the feature map over the eight $D_4$ transforms) to the purely classical \emph{local-raw} feature map instead of the HVK2D-surrogate one, so the comparison is architecture-matched rather than quantum-vs-nothing; the non-symmetric \emph{HVK2D-positional} control; an \emph{ordinary-augmentation} baseline (\emph{HVK2D-augmented}: the same non-pooled feature map, but with a ridge readout trained on all eight $D_4$-transformed copies of every training image, instead of architectural pooling); and the non-symmetric classical \emph{local-raw} control. For every mode we measure both \emph{feature-equivariance error} (as before) and, new here, \emph{output consistency}: for each held-out image we predict a reconstruction from every one of its 8 $D_4$-transformed copies, rotate/reflect each prediction back to canonical orientation, and measure the pairwise mean-squared disagreement between the 8 canonicalized predictions, together with reconstruction accuracy (PSNR) against ground truth.
...
Total: 176 circuits across 5 jobs. Quota consumption, read from \texttt{QiskitRuntimeService.}\allowbreak\texttt{usage()} immediately before and after the pilot: $19\rightarrow35$ seconds against a $600$-second monthly free-plan allowance --- the entire pilot cost $16$ seconds of billed quantum-processor time, with $565$ seconds remaining afterward, at an observed rate of $\approx\!0.09$--$0.13$~s/circuit.
...
The principal artifact map is as follows. Core ablations and held-out controls are generated by \path{Main2/newHVK/run_newhvk_suite.py}, with tabular outputs under \path{Main2/newHVK/results/full_ablation_suite/}, \path{q1_validation/}, and \path{multi_dataset_validation/}. The matched real-circuit topology study is generated by \path{run_topology_comparison.py} and stored under \path{results/topology_comparison/}; its directory includes raw per-run JSON, summaries, paired statistics, and a surrogate manifest. The held-out $D_4$ study is generated by \path{run_d4_symmetry_experiment.py} and stored under \path{results/d4_symmetry_experiment/}. Training-reorganization traces are stored under \path{results/phase_transition_multi_dataset/}. Figure regeneration for the manuscript's reorganization diagnostics is implemented in \path{plot_training_reorganization.py}. Raw and summary artifacts are retained separately so that manuscript means and uncertainty intervals can be recomputed without reading values from figures.

...
reaching 25.90 to 31.52 decibels. HVK adds \del{an entangling observable channel that
...
{\color{red}\textbf{[PI decision pending, title and name. (1) HVK is not a kernel
method in the positive-semidefinite sense, and Section~\ref{sec:intro} contrasts
it with quantum-kernel models. Keep ``Kernel'' and define it, or rename.
(2) ``Symmetry-Pooled'' names a step that is outside the trained path.
(3) ``Hardware Validation'' in the title and keywords names what the body calls
a pilot.]}}
...
HVK focuses on the combination of a Pauli-correlator latent space, enforced
...
the eight symmetries of the square \add{$4\times4$ patch grid} rather than motivating symmetry through
...
only \del{classically} \add{quantum-inspired,} pre-compressed MPS patch features as circuit input, and hands
...
for classification. HVK uses MPS compression only as a \del{classical} \add{quantum-inspired}
...
\begin{enumerate}\itemsep3pt
  ...
  \item \textbf{Competitive with resource-matched classical baselines \add{(held-out comparison)}.} A
        ...
        two one-sided tests (TOST)} on held-out CIFAR-10 shows HVK2D \del{is
...
HVK1D's chain topology serves the complementary role
...
Figure~\ref{fig:hvk_ansatz} shows the exact HVK1D and HVK2D ansatz circuits. These were rebuilt in Qiskit for the hardware pilot of
...
Section~\ref{sec:ablation_relationship}. Since no bit rate is reported, these
results are not a compression claim \citep{Strumpler2022}.}
...
seeds. \add{With five seeds, $p=0.0625$ is the smallest two-sided Wilcoxon
$p$-value possible, so this test cannot reach $0.05$.} The same numerical direction holds on five further real datasets. The
...
terms. The compute needed to produce the features is not matched, since HVK2D
evaluates a quantum circuit for every patch.}
{\color{red}\textbf{[Student: (1) the held-out HVK2D map is 32-D with 6 pair
...
\add{The estimand is the seed-level HVK2D-minus-control PSNR difference,
...
confidence interval lying inside $[-1,+1]$~dB (Table~\ref{tab:tost_main}). This
$t$-based interval is wider than the bootstrap interval above. With only five
seeds the bootstrap interval tends to be too narrow, so the $t$-based interval
is the safer basis. The margin is a declared choice. A 1~dB change in PSNR
corresponds to a factor of about $1.26$ in MSE.}
...
\add{meet the equivalence criterion against} HVK2D within that margin. The two weakest controls, strict classical random
...
{\color{red}\textbf{[PI, not for the text: a Holm correction across the seven
reported TOST $p$-values would keep four equivalences (no entanglement,
$ZZ$-only, quadratic classical, shuffled pair observables) and drop the
raw/local linear control ($p=0.034$), which the abstract names. Any such
reanalysis is the student's to run and report. Otherwise keep the uncorrected
policy stated above.]}}
...
\begin{table}[t]
  ...
  \caption[TOST equivalence test on held-out CIFAR-10]{\add{TOST equivalence
           ...
           seed-level paired differences, HVK2D minus control). Equivalence
  ...
  \begin{tabular}{@{}l r c r l@{}}
    ...
    Strict classical random features & $+2.27$ & $[+1.92,+2.63]$ & $0.999$   & HVK2D better \\
    Random VQC                       & $+6.97$ & $[+5.56,+8.39]$ & $1.000$   & HVK2D better \\
    \bottomrule
  \end{tabular}
...
exercise} \add{hardware replay}, reported in the companion Supplementary Material. Fresh HVK1D
...
rotation symmetry into the circuit itself \citep{SanSebastianSein2025}. In HVK
the circuit is not equivariant. The symmetry comes from averaging the observable
grid over the group after measurement, and the same average applies equally to
a classical feature map.}
...
report the result across five seeds. HVK2D's \del{entangling observable}
...
\begin{table}[t]
  \centering
  \caption[HVK2D restricted pair-correlation diagnostic]{HVK2D restricted pair-correlation diagnostic across five seeds.
  ...
  \begin{tabular}{@{}l r r r@{}}
    ...
    \midrule
    HVK2D \del{entangling} \add{pair} observables     & $8.5969\times10^{-4}$     & 32.77          & $ 0.9735$  \\
...
three questions. First, HVK at six qubits can be simulated exactly on a
...
by a classical method based on Pauli shadows \citep{Bermejo2026}. Third, on the tested ordinary reconstruction tasks we observe no
quantum benefit. This matches large benchmarks in which simple classical models
often match or beat small quantum models, and removing entanglement often costs
little \citep{Bowles2024}.} \add{In HVK the
...
per-image-trained HVK2D models \del{reconstruct with high fidelity}
...
real quantum hardware.} \add{At the six-qubit scale tested here, HVK is

...
held-out comparison using CIFAR-10, a pre-declared TOST equivalence procedure at
a one-decibel margin establishes that HVK2D is statistically \emph{competitive}
...
reaching 25.90 to 31.52 decibels. HVK adds an entangling observable channel that
is representationally necessary on a leakage-audited task requiring
distant-patch products, an interpretable Hamiltonian energy diagnostic, and
native hardware execution. We make no claim of quantum advantage.}
...
route to representations with favourable inductive bias, exploiting entanglement,
physically motivated regularisers, or tensor-network structure beyond what
classical convolutional pipelines provide
...
\citep{Cerezo2021, Tilly2022, Bauer2020}. Quantum image processing provides
...
\citep{Chakraborty2018, Zhou2017, Yao2017}. Tensor-network methods have proven
effective as compressed representations of structured classical data, including
images
\citep{Cirac2021, Latorre2005, Han2018, Stoudenmire2016, Ran2020TNCS, Guala2023}
and particle-physics events \citep{Araz2022}. Related hybrid architectures target
adjacent imaging tasks, including tomographic reconstruction
\citep{Haga2024, Nau2023}, ghost imaging \citep{Zhai2025}, and quantum
convolutional and capsule networks
\citep{Slabbert2024, Cong2019, Hur2022, Liu2022}. HVK focuses on the specific
combination of a Pauli-correlator latent space, enforced group-equivariant
pooling, and a physically motivated energy diagnostic.
...
The second is an \emph{exact, numerically-verified $D_4$-equivariant
observable-pooling construction}, which group-averages the observable grid over
the eight symmetries of the square rather than motivating symmetry through
...
nearest-neighbour $XX/YY/ZZ$ sectors in HVK1D and grid-edge $ZZ$ terms in HVK2D.
...
fidelity objective. HVK never amplitude-encodes pixels into qubits. It exposes
...
method. HVK reads out an explicit vector of local and pairwise Pauli expectation
values as a latent representation for a trained decoder, and adds enforced
group-equivariant pooling and a Hamiltonian regulariser that a kernel
...
HVK's contribution is the assembly of these four elements in one pipeline
...
\begin{enumerate}\itemsep3pt
  ...
  \item \textbf{Competitive with resource-matched classical baselines.} A
        ...
        statistically equivalent, within a $\pm1$~dB margin, to \snew{five of seven
  ...
  \item \textbf{Equivariant observable pooling.} An exact,
        ...
        rather than from anything quantum. The identical construction applied to
        a purely classical feature map yields the same exactness (Supplementary
...
\color{blue}
HVK treats each spatial patch as a finite spin state and compresses it as a
...
the primary learned object \citep{Shi2022}; in HVK it is instead a diagnostic
...
$\Phi(x_p)\in\mathbb{R}^{d}$ ($d=27$ for HVK1D, $19$ for HVK2D). {\color{red}%
With $n=6$ qubits, bond set $\mathcal{E}$, and $c$ measured pair channels per bond,
\begin{equation}
  \label{eq:obs_dim}
  d=2n+c\lvert\mathcal{E}\rvert=
  \begin{cases}
    2(6)+3(5)=27, & \text{HVK1D ($ZZ,XX,YY$ on a 5-bond chain)},\\
    2(6)+1(7)=19, & \text{HVK2D ($ZZ$ on 7 grid edges)},\\
    2(6)+1(5)=17, & \text{$ZZ$-only ablation control}.
  \end{cases}
\end{equation}
All reported HVK1D and HVK2D results use $d=27$ and $d=19$.}
A classical
...
correlations are cheap classical features that do not require a full
...
interface at $O(\text{qubits})$ rather than exponential readout cost. It also
...
\paragraph{Why a grid rather than a chain interaction topology for HVK2D.}
Aligning the six-qubit interaction graph with the $2\times3$ patch-neighbour
adjacency, rather than an arbitrary linear chain, is what makes the $D_4$
...
two-dimensional adjacency. HVK1D's chain topology serves the complementary role
in this design: it is the ungridded control against which the grid's
contribution is measured (Section~\ref{sec:topology}).
...
Table~\ref{tab:variants} summarises the tested variants. HVK1D uses a linear
6-qubit chain (5 nearest-neighbour bonds, 27-D observable vector); HVK2D arranges
...
\textit{Monalisa} protocol developed for HVK: a fresh model is optimised directly
on its target image, with no held-out split. These results therefore measure
...
and restricted-entanglement diagnostics use the separate protocols stated in
...
For each image, we trained a fresh HVK2D model using non-overlapping $8\times8$
...
\subsection{Scope: models do not transfer zero-shot}
\label{sec:no_generalization}

A model optimised on one image alone does not transfer zero-shot to a
...
HVK's feature map improves reconstruction on real, \emph{held-out} images under
a resource-matched multi-seed protocol. That question is answered in the
companion document, \textit{Supplementary Material for the Hamiltonian Vision
Kernel: Resource-Matched Ablations, Validation, and Reproducibility}.

On held-out CIFAR-10, \snew{the raw/local linear control} reaches
$18.80\pm1.42$~dB, against HVK2D's $18.12\pm1.54$~dB. Aggregating held-out
images within each split gives an HVK2D-minus-control difference of $-0.68$~dB,
...
tested HVK map is therefore informative, but it is not shown to outperform the
simpler maps on ordinary held-out reconstruction.

A TOST equivalence test at a declared $\pm1$~dB margin confirms that this gap is
small enough, in a pre-declared and data-independent sense, to call the two
\emph{competitive} rather than merely ``not shown to differ''.
\snew{Five of seven distinct} tested controls are statistically equivalent to
HVK2D within that margin. The two weakest controls, strict classical random
features and random-VQC, are correctly excluded, because HVK2D substantially
outperforms them. We report this as a complement to the positive entanglement
...
ties-with-classical result and the entanglement-necessity result of
...
target image following the same per-image protocol as every result in this
paper. For each image we took an already-trained HVK1D/HVK2D checkpoint's exact
...
Five images constitute a pilot, and we make no hardware-advantage claim. What
the pilot does establish is that the trained HVK observable channel survives
execution on present-day hardware. The learned parameters, the measurement
...
unmitigated device. This is the sense in which HVK runs on real quantum
...
The same checkpoint-replay methodology supports a further hardware-validation
exercise, reported in the companion Supplementary Material. Fresh HVK1D
...
images. We attribute this residual to drift between calibration and execution,
crosstalk, and correlated errors. The airplane image is the clear outlier, with
...
under $1\%$ of the monthly limit. Two checkpoints, \textit{Monalisa} and CIFAR
...
(Section~\ref{sec:hardware_robustness}). It provides direct evidence, not merely
...
\end{equation}
up to floating-point error. We verify this numerically over 7000 patch
...
The diagnostic tests whether an entangling pair-observable channel is needed to
...
receives the same raw inputs as HVK2D. {\color{red}\textbf{The task is by
...
report the result across five seeds. HVK2D's entangling observable channel
...
$\langle O_i\rangle\langle O_j\rangle$ that the target depends on. The entangling
measurement supplies the joint correlator $\langle O_iO_j\rangle$ directly as a
feature. An entangling pair-observable channel is therefore
\emph{representationally necessary within the tested feature class and fixed
linear-readout protocol} to fit this target. A sufficiently nonlinear classical
readout could in principle recover the same cross-terms, which is why we scope
the claim to the linear protocol rather than to classical models in general.
...
it does not track that term. We make no transition claim, and report these traces
...
\subsection{Why the physics-informed regulariser was inert}
...
Whether a quantum feature map offers any advantage over a classical model is a property of the specific data and target, not a fixed property of the model class. Dequantization results have repeatedly shown that apparent quantum advantages on structured, low-entanglement classical data can be matched once the right classical feature is identified \citep{Huang2021PowerData, Tang2019}. Where provable separations do exist, they are established for carefully constructed learning problems rather than for generic structured data \citep{Gyurik2023}.

The restricted diagnostic of Section~\ref{sec:entanglement_necessity} is constructed so that the relevant structure is a genuine distant-element product, which raw or local features cannot represent under a fixed linear readout. That is the regime in which HVK's entangling channel separates. Natural image patches at the resolution and patch size used here are low-entanglement, locally structured data, where a linear or shallow classical readout is already close to sufficient. The distinction lies in the measured pair-observable channel rather than in the circuit. The ansatz uses strongly-entangling layers and does generate an entangled state, but whether the measured channel carries task-relevant information is a property of the target. On natural-image targets the pair observables add little beyond the local ones. On the constructed nonlocal target they are necessary. Together with the held-out comparisons of Section~\ref{sec:ablation_relationship}, these results delineate where the tested entangling representation does and does not yield a measurable benefit.

HVK's three ingredients each recur individually in the hybrid quantum-vision literature, covering MPS compression of structured input \citep{Stoudenmire2016, Han2018, Cirac2021, Guala2023}, a Pauli-correlator latent space handed to a classical decoder \citep{Havlicek2019}, and a physically motivated auxiliary regulariser alongside a task loss \citep{Shi2022}. Its specific instantiation, a six-qubit VQC with a $19$--$27$-D correlator vector and a graph-Hamiltonian term, sits within the parameter ranges typical of that literature. The measured behaviour is therefore evidence about the design pattern itself. We do not claim the boundary transfers unchanged to every dataset, patch size, or qubit count.

\subsection{Topology alignment as an inductive bias}
\label{sec:topology}

Aligning the latent interaction graph with image-patch adjacency (HVK2D vs.\ HVK1D) is an architectural inductive bias, and the companion supplementary study tests it directly under matched held-out conditions. Its real-circuit subset (3 seeds, 3 held-out CIFAR-10 images) gives an HVK1D-minus-HVK2D mean PSNR difference of $0.16$~dB. After averaging images within seed, the bootstrap 95\% CI is $[-0.38,0.46]$~dB and the seed-level Wilcoxon test gives $p=0.50$. A broader five-seed surrogate comparison across CIFAR-10, Fashion-MNIST, and PathMNIST likewise finds numerically small topology differences, at most $0.07$~dB in mean PSNR. HVK2D completes the matched 90-step runs in roughly half the wall-clock time. At the tested scale, topology is therefore primarily a resource-cost choice rather than a reliable accuracy advantage. These tests do not establish formal equivalence.
...
\item Every reconstruction result in this paper (Sections~\ref{sec:sameset_results}--\ref{sec:no_generalization}) is per-image trained. The companion supplementary study's held-out protocol is the appropriate evidence for a generalisation claim.
...
\item The entanglement-necessity result (Section~\ref{sec:entanglement_necessity}) is scoped to a task constructed to require nonlocal structure. Ordinary natural-image reconstruction lies outside that scope (see Section~\ref{sec:dequant}).
\item The hardware pilot (Section~\ref{sec:hardware_pilot}) is a five-image, single-trial, unmitigated pilot rather than a statistically resolved hardware benchmark.
...
We presented the Hamiltonian Vision Kernel, a hybrid quantum--classical
...
per-image-trained HVK2D models reconstruct with high fidelity. On a
...
establishing feasibility at a five-image, single-trial scope
(Section~\ref{sec:hardware_pilot}).

HVK's case rests on capabilities a classical reconstruction pipeline does not
provide. Pair observables are necessary on a task requiring nonlocal structure,
within the tested fixed-readout feature class
...
We make no claim of quantum advantage. HVK is, at this scope, a new architecture
...
search for further real-world tasks with nonlocal structure where HVK's
...
\begin{description}\itemsep4pt
  ...
  \item[Code availability.] The source code, experiment drivers,
        ...
        resource-matched held-out comparison, on which the tested HVK map is
        competitive with rather than superior to the classical controls, and the
        withdrawn training-dynamics material. Automated tests verify those
        mappings.

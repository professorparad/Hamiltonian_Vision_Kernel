...
fidelity objective. PCAE never amplitude-encodes pixels into qubits. It exposes
...
PCAE's contribution is the assembly of these four elements in one pipeline
...
PCAE1D's chain topology serves the complementary role
...
tested PCAE map is therefore informative, but it is not shown to outperform the
...
paper. For each image we took an already-trained PCAE1D/PCAE2D checkpoint's exact
...
the pilot does establish is that the trained PCAE observable channel survives

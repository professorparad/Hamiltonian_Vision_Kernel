...
full.
\color{green}
The remaining stages are common to both. A learned linear layer compresses the
MPS feature vector to six circuit angles, one per qubit. After the circuit is
executed, a two-hidden-layer MLP (128 and 256 units) maps the measured
observables together with the Fourier positional vector back to a single
greyscale patch. The positional encoding follows established use as a spatial
inductive bias in classical generative vision models \citep{Xu2021}.
Figure~\ref{fig:hvk_ansatz} shows the exact HVK1D and HVK2D ansatz circuits.
These were rebuilt in Qiskit for the hardware pilot of
Section~\ref{sec:hardware_pilot}, and numerically verified against the
training-time PennyLane circuit before use. The complete Hamiltonian, loss,
...
loss; because learned energy can go negative, the optimiser can lower total loss
...
reconstruction loss already dominates the optimisation signal at this model scale
and dataset size: a physically motivated regulariser has room to help when it
...
gradient. This does not mean Hamiltonian regularisation is never useful; its
...
held-out comparisons (Section~\ref{sec:ablation_relationship}) characterise
...
inert regulariser (Section~\ref{sec:regularizer_inert}): that some idiosyncratic
implementation choice --- an unmotivated qubit count, an unusual latent width, or
a peculiar regulariser formulation --- rather than the design pattern itself, is
...
motivated auxiliary regulariser alongside a task loss \citep{Shi2022} --- and its
...
fair, properly-scaled instance rather than a strawman, and its measured behaviour
...
\begin{itemize}\itemsep4pt
  ...
  \item The training-dynamics traces (Section~\ref{sec:phase_transition}) are an
        ...
        Hamiltonian-regularised model (4/4 vs.\ 4/4, full table in the

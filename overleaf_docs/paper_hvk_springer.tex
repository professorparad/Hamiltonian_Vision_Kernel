...
and 3 seeds per variant, and replaces an earlier table whose per-seed provenance
could not be recovered. A plain classical-replacement control ties or slightly
...
optimised directly on its own target image following the same per-image protocol
...
cases --- exactly as often as the Hamiltonian-regularised model's $4/4$ --- so we

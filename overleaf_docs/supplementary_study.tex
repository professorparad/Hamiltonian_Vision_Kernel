...
\subsection{Repeated-Execution Anchor Jobs}
\label{sec:supp_hardware_anchors}
The main paper's repeated-execution anchor points --- the same trained checkpoints,
replayed without retraining on two backends other than \texttt{ibm\_fez} --- were four
further jobs, listed in Table~\ref{tab:hardware_anchor_jobs} so that every real-hardware
number in the main text has a job identifier here. Submission settings match the pilot
above (\texttt{SamplerV2}, \texttt{optimization\_level=1}); only backend and shot count
differ. The accompanying shot-budget sweep reported alongside them in the main text is a
calibrated device-noise \emph{simulation} (\texttt{qiskit-aer} with the \texttt{FakeFez}
calibration snapshot) and consumed no quantum-processor time, so it has no job identifier.

\begin{table}[H]
\centering
\caption{Anchor-job identifiers for the repeated-execution checks. Job identifiers,
backends, shot counts and PSNR values are read from the retained execution record
\texttt{IBM\_Cloud/outputs/hardware\_robustness\_study/real\_hardware\_anchors.json};
the PSNR column matches the main text.}
\label{tab:hardware_anchor_jobs}
\scriptsize
\begin{tabular}{lllcrc}
\toprule
Image & Backend & Job ID & Shots & Circuits & PSNR (dB) \\
\midrule
Monalisa (HVK1D)  & \texttt{ibm\_marrakesh} & \texttt{d9gqm58gk0ls73f219m0} &  256 & 48 & 25.94 \\
Monalisa (HVK1D)  & \texttt{ibm\_kingston}  & \texttt{d9gqnk0gk0ls73f21bkg} & 1024 & 48 & 26.10 \\
CIFAR cat (HVK2D) & \texttt{ibm\_kingston}  & \texttt{d9gqqi0gk0ls73f21glg} &  256 & 32 & 28.93 \\
CIFAR cat (HVK2D) & \texttt{ibm\_kingston}  & \texttt{d9gqr8chonhs73ac3fmg} & 1024 & 32 & 31.24 \\
\bottomrule
\end{tabular}
\end{table}
